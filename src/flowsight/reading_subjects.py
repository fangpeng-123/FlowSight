...
def load_catalog(
    ...
    *,
    ...
    if not catalog_path.is_file():
        ...
        incoming = _catalog_from_dict(project, raw)

    subjects = _merge_locked(previous, incoming)
    _validate_unique(subjects)
    return ReadingSubjectCatalog(version=CATALOG_VERSION, subjects=subjects)
...
def apply_catalog(doc: S.GraphDocument, catalog: ReadingSubjectCatalog) -> None:
    ...
    for subject in catalog.subjects:
        module = modules_by_root.get(subject.root)
        if module is None:
            raise CatalogValidationError(
                f"subject root {subject.root!r} does not map to a parser module"
            )
        ...
        module.attrs["reading_subject"] = subject.to_dict()
